...
The code distribution contains the following files:
  ...
  * big_num.py - pset (stripped) versions of large number arithmetic
  ...
  * test/README.txt - describes how to generate tests
...
big_num.py's behavior can be tweaked using the MUL environment variable, which
chooses the multiplier algorithm in the library. The following values are
recognized:
* kara - Karatsuba
* slow (default) - O(N^2) quadratic multiplication

Command-line example:
    MUL=kara python rsa.py < tests/1gate.in
