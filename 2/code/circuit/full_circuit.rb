...
#
# This is a devil's-advocate N^2 implementation. It takes advantage of the fact
# that array operations are done in native code (most likely in a memmove call),
# and so they're much faster than high-level statements in a log(N)
# implementation.
class BlitPriorityQueue < PriorityQueue
  # Initially empty priority queue.
  def initialize
    @queue = []
  end
  ...
  def <<(key)
    ...
    while low <= high
      ...
      # The comparison is flipped because the array is sorted in reversed order.
      if cmp < 0
        low = mid + 1
      else
        high = mid - 1
...
end  # class BlitPriorityQueue

# Instrumented version of BlitPriorityQueue.
# implementation.
class BlitStatPriorityQueue < BlitPriorityQueue
  # Initially empty priority queue.
  def initialize
    super
    @lblit_work = 0
  ...
  def <<(key)
    ...
    @lblit_work += high + 1
    @rblit_work += @queue.length - (high + 1)
    @queue.insert high + 1, key
    self
  end
  
  # :nodoc: this gets called by PriorityQueueStats#stats
  def extra_stats
    { :lblit_avg => @lblit_work / @insert_count.to_f,
      :rblit_avg => @rblit_work / @insert_count.to_f }
  end
end  # class BlitStatPriorityQueue

# Priority queue implementation based on rbtree or rbtree-pure rubygem.
# implementation.
...
class HeapPriorityQueue < PriorityQueue
  ...
  def shift
    ...
    popped_key = @heap[1]
    return @heap.pop if @heap.length == 2
    @heap[i = 1] = key = @heap.pop
    loop do
      break unless left_key = @heap[left = i * 2]
      if (right_key = @heap[right = i * 2 + 1]) && right_key < left_key
        child_key = right_key
        child = right
      else
        child_key = left_key
...
class Simulation
  ...
  #   :queue:: selects between one of the following implementations:
  #            :heap:: an O(log(N)) / operation queue
  #            :blit:: an O(N) / operation queue which uses O(log(N)) Ruby
  #                    statements, but moves O(N) bytes via array ops
  #            :slow:: an O(N) / operation queue which uses O(N) Ruby statements
...
class Cli
  def run(args)
    ...
    sim.queue(:queue => (ENV['QUEUE'] || 'slow').downcase.to_sym,
        :stats => (ENV['TRACE'] == 'stats'), :debug => (ENV['DEBUG'] == 'true'))
    sim.run
    if sim.has_stats?
      sim.print_stats
