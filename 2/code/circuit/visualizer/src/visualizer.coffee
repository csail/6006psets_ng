...
class Trace
  ...
  #
  # The trace timeline is not continuous. This method only works for times that
  # are on the timeline. When in doubt, call nearestEventTime before calling
  # this method.
  #
  # @param changeBag initial value for the object recording changed gates;
  #                  passing something other than {} can be useful for merging
...
class LayoutView
  ...
  # Updates the view to reflect a change in a gate's value.
  updateGate: (gate) ->
...
class Visualizer
  constructor: (svgContainer, traceData) ->
    @circuit = new Circuit traceData.circuit
    @trace = new Trace traceData.trace, @circuit
    @view = new LayoutView svgContainer, traceData.layout, @trace
    @frameTimeout = null
