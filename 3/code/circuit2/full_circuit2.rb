...
# :nodoc: Efficient sweeping line algorithm.
class CrossVerifier
  # Populates the sweep line events from the wire layer.
  def events_from_layer(layer)
    layer.wires.each do |wire|
      if wire.horizontal?
        @events << [wire.x1, 0, wire.id, :add, wire]
        @events << [wire.x2, 2, wire.id, :remove, wire]
      else
        @events << [wire.x1, 1, wire.id, :query, wire]
      end
    end
  ...
  def compute_crossings!(count_only = true)
    ...
    @events.each do |sweep_x, _, _, event_type, wire|
      trace_sweep_line sweep_x
      case event_type
      when :add
        @index.add KeyWirePair.new(wire.y1, wire)
      when :remove
        @index.delete KeyWirePair.new(wire.y1, wire)
      when :query
        range = KeyWirePairL.new(wire.y1)..KeyWirePairH.new(wire.y2)
        if count_only
          result += @index.count range
        else
          @index[range].each do |kwp|
...
end  # class CrossVerifier -- optimized version
